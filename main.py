...
def determine_use_case(name):
    ...
    elif any(kw in name_lower for kw in ["vision", "vl", "llava", "pixtral"]):
        return "[bold magenta]Vision[/bold magenta]"
    elif any(kw in name_lower for kw in ["math"]):
        return "[bold cyan]Math[/bold cyan]"
    elif any(kw in name_lower for kw in ["reasoning", "think", "-r1", "deepseek-r1"]):
        return "[bold yellow]Reasoning[/bold yellow]"
    elif any(kw in name_lower for kw in ["embed", "bge", "nomic"]):
        return "[grey74]Embedding[/grey74]"
    elif any(kw in name_lower for kw in ["instruct", "chat", "dolphin", "hermes"]):
...
def calculate_fit(size_gb, specs):
    ...
    elif specs["has_gpu"] and (specs["vram_free"] + specs["ram_free"]) >= req:
        return (
            "[bold yellow]Partial[/bold yellow]",
            "[yellow]GPU+CPU[/yellow]",
            "VRAM+RAM",
        )
    elif specs["ram_free"] >= req:
...
class ModelDetailModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.data["source"] == "Ollama":
            ...
            cmd_text = (
                f"huggingface-cli download {self.data['name']} --include '*.gguf'"
            )
            ...
            if resp.status_code == 200:
                ...
                for a_tag in soup.find_all("a", href=True):
                    ...
                    if (
                        ...
                        pulls = re.search(
                            r"(\d+(?:\.\d+)?[KM]?)\s*Pulls", full_text, re.IGNORECASE
                        )
                        if not pulls:
